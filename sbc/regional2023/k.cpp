...
vector<ll> fat; // factorial
vector<ll> invfat; 

inline ll modulo(ll k){
	return ((k%mod)+mod)%mod;
}

vector<ll> f(vector<ll> &p, int k){
	int n = p.size();
	vector<ll> A(n), B(n);
	int aux = 1;
	for(int i = 0; i<n; i++){
		A[i] = ((int128)p[i]*fat[i])%mod;
		B[i] = ((int128)aux*invfat[i])%mod;
		aux = ((int128)aux*k)%mod;
	}
	...
	vl D = C;
	for(int i = 0; i<=n; i++){
		D[i] = ((int128)C[n-i-1]*invfat[i])%mod;
	}
	// debug(D);
	return D;
}
...
int main(){
	...
	invfat[0] = 1;
	for(int i = 1; i<=n; i++){
		fat[i] = ((int128)i*fat[i-1])%mod;
		invfat[i] = modpow(fat[i], mod-2);
	}

	vl pk = f(p, k), tk = f(t, modulo(-k));
	// debug(pk);
	// debug(tk);
