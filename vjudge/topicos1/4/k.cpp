...
class SegTree{
	private:
		...
		ll pos_pop(int i, int l, int r, int pos){
			if(l == r){
				update(1, l, r, 0, n-1, 0);
				return l;
			}
			int mid = (l+r)/2;
			if(St[rl(i)] >= pos){
				return pos_pop(rl(i), l, mid, pos);
			} else {
				return pos_pop(rr(i), mid+1, r, pos-St[rl(i)]);
			}
		}
	...
	public:
		...
		ll pos_pop(int pos) {
			if (pos > St[1]) return -1;
			return pos_pop(1, 0, n-1, pos);
		}
...
int main(){
	...
	for(int i = 0; i<n/2; i++){
		res1[i] = seg.pos_pop(p1[i]);
		res2[i] = seg.pos_pop(p2[i]);
	}
	for(int i = 0; i<n/2; i++){
		if(i) cout<<" ";
		cout<<res1[i]+1;
